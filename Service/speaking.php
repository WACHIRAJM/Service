...
$nameErr = $emailErr = "";
$name = $phone = $email = $org_name = $title = $number = $date = $time = $location = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
 ...
 if (empty($_POST["title"])) {
    $title = "please input your title here";
  } else {
...
<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">  
  ...
  <br><br>
  Event Title:</br> <textarea name="title" rows="1" cols="20"><?php $title;?></textarea>
  <br><br>
  What is the size of your group?</br> <input type = "number" name="number" value="><?php $number;?>">
  <br><br>
...
if(isset($_POST['submit']))
{
	...
	if($name == '' or $phone == '' or $email == '' or $org_name == '' or $title == ''or $number == '' or $date == ''or $time == '' or $location =='')
{
	echo "<script>alert('please fill all the required fields')</script>";
	exit();
}else{
	$insert_querry ="INSERT INTO speaking(name,phone,email,org_name,title,number,date,time,location)
	                           VALUES('$name','$phone','$email','$org_name','$title','$number','$date','$time','$location')";
